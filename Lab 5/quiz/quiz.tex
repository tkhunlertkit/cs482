...
\begin{questions}
	\question[5] This image file \texttt{dice.jpg} is 120 x 80 pixels in size. It is composed of 6 40 x 40 sub-images.
		\begin{figure}[ht!]
			...
			\begin{tikzpicture}
				\node[block] (1) {1};
				\node[block, right of=1, node distance=3.1em] (2) {2};
				\node[block, right of=2, node distance=3.1em] (3) {3};
				\node[block, below of=1, node distance=3.1em] (4) {4};
				\node[block, right of=4, node distance=3.1em] (5) {5};
				\node[block, right of=5, node distance=3.1em] (6) {6};
			\end{tikzpicture}
		\end{figure}
	\\
